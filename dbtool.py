...
class Tool:
    def __init__(self, tmpDir: Optional[Path] = None, userFileName: str=''):
        if tmpDir is None:
            tmpDir = pth.tmp_path()
        self._tmp = tmpDir
    ...
    def _create_engine(self):
        parser = ConfigParser()
        _ = parser.read(pth._get_tool_path() / 'secret/db.cfg')
    ...
    def _get_crs(self, table_name: str, geo_col: str, schema: str='test_loic') -> str:
    ...
    def fetch_query(self, query: str, storeIt: bool=True, geo_col: Optional[str]=None, geo_table_name: Optional[str] = None, force_refetch: bool=False, params: Optional[Dict[str, Any]]=None) -> Union[pd.DataFrame, pdg.GeoDataFrame]:
        ...
        if geo_col is not None:
            if geo_table_name is None:
                ...
                crs = 'EPSG:' + self._get_crs(geo_table_name, geo_col)
