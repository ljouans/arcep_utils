...
def _read_single_file(filelike: IO[bytes], cols: Optional[List[str]] = None, nrows: Optional[int]=None):
    ...
    for encoding in all_encodings:
        try:
            ...
            df = pd.read_csv(filelike, sep=';', decimal=',', encoding=encoding, usecols=take_col, dtype=str, nrows=nrows) # type: ignore

            if df.shape[1] > 0:
                break
                
            logging.debug('Not enough columns with encoding %s. Trying with some other.', encoding)

        except UnicodeDecodeError:
            logging.debug('Encoding %s did not work. Trying anew', encoding)
        filelike.seek(0)
    return df
...
def read_all_ipe(columns: List[str], numeric_cols: List[str] = None, cols_are_optional: bool = True, _test_nrows:int=None):
    """
...
def read_single_ipe( ipe_name: str, columns: List[str], numeric_cols: List[str]= None, zipfilepath: Optional[Union[str, Path]] = None, _test_nrows: int = None):
